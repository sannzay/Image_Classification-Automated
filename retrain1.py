...
def main(_):
  ...
  for i in range(FLAGS.how_many_training_steps):
    ...
    #skips the check on first iteration
    if (te == -100):
      te = 0
      continue
    #stops when the previous step validation accuracy is greater then the current one
    elif (te > validation_accuracy) :
      break 
    #When this value hits negative, training stops
    # print(validation_accuracy - te)
    #assigning the current validation score to compare with the next step's validation score   
    te = validation_accuracy
